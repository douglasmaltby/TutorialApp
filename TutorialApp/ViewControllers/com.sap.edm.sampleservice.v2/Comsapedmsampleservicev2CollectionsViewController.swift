...
protocol Comsapedmsampleservicev2EntityUpdaterDelegate {
    func entityHasChanged(_ entity: EntityValue?)
}

protocol Comsapedmsampleservicev2EntitySetUpdaterDelegate {
    func entitySetHasChanged()
}

class Comsapedmsampleservicev2CollectionsViewController: FUIFormTableViewController {
    private var collections = Comsapedmsampleservicev2CollectionType.all
    ...
    private func collectionSelected(at indexPath: IndexPath) {
        ...
        guard let odataController = OnboardingSessionManager.shared.onboardingSession?.odataControllers[destinations["com.sap.edm.sampleservice.v2"] as! String] as? Comsapedmsampleservicev2OnlineODataController, let espmContainer = odataController.espmContainer else {
    ...
    private func selectDefault() {
        ...
        guard let odataController = OnboardingSessionManager.shared.onboardingSession?.odataControllers[destinations["com.sap.edm.sampleservice.v2"] as! String] as? Comsapedmsampleservicev2OnlineODataController else {
            AlertHelper.displayAlert(with: "OData service is not reachable, please onboard again.", error: nil, viewController: self)
            return
        }

        if self.splitViewController!.isCollapsed || odataController.espmContainer == nil {
            return
        }

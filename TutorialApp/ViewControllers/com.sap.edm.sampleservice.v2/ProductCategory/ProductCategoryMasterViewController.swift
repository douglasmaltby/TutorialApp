...
class ProductCategoryMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([ProductCategory]?, Error?) -> Void) -> Void)?
    private var entities: [ProductCategory] = [ProductCategory]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let productCategory = self.entities[indexPath.row]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].category ?? ""
...
extension ProductCategoryMasterViewController: Comsapedmsampleservicev2EntitySetUpdaterDelegate {

...
class PurchaseOrderItemMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([PurchaseOrderItem]?, Error?) -> Void) -> Void)?
    private var entities: [PurchaseOrderItem] = [PurchaseOrderItem]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let purchaseOrderItem = self.entities[indexPath.row]
...
extension PurchaseOrderItemMasterViewController: Comsapedmsampleservicev2EntitySetUpdaterDelegate {
